...
class OpenAIClient(LLMClient):
    ...
    async def suggest_wholesale_mapping(
        ...
        prompt = (
            ...
            "When comma-separated absolute option prices align one-to-one with option_values_raw "
            "(for example, 2000,3000), map that source only to price_wholesale_raw and do not map it "
            "to option_price_deltas_raw; the downstream parser uses the first price as the base "
            "wholesale price and computes deltas (0,1000).\n"
