...
def test_smartstore_adapter_builds_payload_and_pricing_snapshot():
    result = SmartstoreAdapter().build_draft(
        source_snapshot=_source_snapshot(),
        account_settings=_settings(),
    )
...
def test_coupang_adapter_uses_market_specific_policy_and_keeps_item_assets():
    result = CoupangAdapter().build_draft(
        source_snapshot=_source_snapshot(),
        account_settings=_settings(),
    )
    ...
    assert payload["items"][0]["contents"][0]["contentType"] == "HTML"
...
def test_missing_category_and_image_are_blocking_for_both_adapters():
    ...
    smartstore_result = SmartstoreAdapter().build_draft(
        source_snapshot=snapshot,
        account_settings=_settings(),
    )
    coupang_result = CoupangAdapter().build_draft(
        source_snapshot=snapshot,
        account_settings=_settings(),
    )
...
def test_missing_and_invalid_pricing_policy_are_market_specific_blocks():
    ...
    missing_policy_smartstore = SmartstoreAdapter().build_draft(
        source_snapshot=_source_snapshot(),
        account_settings=settings_without_pricing,
    )
    missing_policy_coupang = CoupangAdapter().build_draft(
        source_snapshot=_source_snapshot(),
        account_settings=settings_without_pricing,
    )
    assert _error_codes(missing_policy_smartstore) == ["SMARTSTORE_MISSING_PRICING_POLICY"]
    assert _error_codes(missing_policy_coupang) == ["COUPANG_MISSING_PRICING_POLICY"]
    ...
    invalid_policy_smartstore = SmartstoreAdapter().build_draft(
        source_snapshot=_source_snapshot(),
        account_settings=invalid_settings,
    )
    invalid_policy_coupang = CoupangAdapter().build_draft(
        source_snapshot=_source_snapshot(),
        account_settings=invalid_settings,
    )
    assert _error_codes(invalid_policy_smartstore) == ["SMARTSTORE_INVALID_PRICING_POLICY"]
    assert _error_codes(invalid_policy_coupang) == ["COUPANG_INVALID_PRICING_POLICY"]
