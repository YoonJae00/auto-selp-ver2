...
The active key is bound to a private normalized identity tuple of supplier name, main URL, login URL, and login-enabled state. Form changes invalidate the reference before any later picker, test, or save operation. Unsaved `studio-…` credentials are best-effort deleted on invalidation, while migrated runtime slug credentials are preserved but are no longer eligible for reuse. Credential loading and migration require both an exact identity match and login to remain enabled.
...
- Change supplier name, site URL, login URL, and login-enabled state in the same view model and assert credentials never cross identities.
