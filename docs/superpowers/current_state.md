...
1. GitHub #43: trademark UX cleanup
   - Move LLM-suspected keyword information into the Intelligence Capsule trace view.
...
8. Wholesale Product Upload Rename & Excel Preview (Completed)
...
- Keyword curation with Naver Search AD data, blacklist filtering, and LLM brand classification.
- LLM-suspected brand keywords are excluded with `llm_suspected` warnings.
...
- Process page with Excel upload, column mapping, LLM provider usage, and background task creation.
- Settings page for global LLM provider and persisted column mapping.
...
- **Real-Time Attribute UI Integration (NEW)**: Connected backend LangGraph attribute extraction stage results reactively to the frontend. Added the 4th stage `'extracting'` (속성 추출) to the floating `IntelligenceCapsule` drawer and detail views, displaying live attribute count summaries. Rewrote the table layout's `renderAttributes` helper using a DRY, O(1) lookup, exception-resistant parser (`extractAttributesList`) that binds live Zustand store changes reactively, populating Coupang and Naver attribute tags instantly as they are extracted before final DB re-fetches.
...
- **Product Columns Visibility & Reordering (NEW)**: Implemented dynamic column visibility toggling, native drag-and-drop column reordering, localStorage persistence (`autoselp_product_columns_config`), outside click popover dismissal, and fully responsive 17-column dynamic rendering.
...
- Brand-suspicion trace UX is only partially inline today: completed keyword stages show filtered keywords but do not show an explicit no-issue state.
